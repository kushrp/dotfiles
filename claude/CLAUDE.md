...
Use Shrek's format for every PR description, including updates. Read
`~/.agents/skills/pr-description/SKILL.md` (also exposed as the `pr-description`
skill in Codex). Start with `Why this change`, `What this stack does`, and
`What this PR does`. Repeat the first two sections verbatim across a stack.
Omit `What this stack does` for standalone PRs. Follow with `Human Instructions`,
`Model Assumptions`, `Test Evidence`, tracking links, and a final risk block.
Add a glossary or review map when useful. Follow required repository sections
and ordering.
